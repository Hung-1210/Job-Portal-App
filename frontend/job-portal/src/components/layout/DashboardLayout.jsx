import { Children, Profiler, useEffect, useState } from 'react';
import {
  Briefcase,
  Building2,
  LogOut,
...
const NavigationItem = ({ item, isActive, onClick, isCollapsed}) => {
     ...
     return <button onClick={() => onClick(item.id)}
  ...
  return <div className="flex h-screen bg-gray-50">
    ...
    >
        ...
        <div className="flex items-center h-16 border-b border-gray-200 pl-6">
            {!sidebarCollapsed ? (
                ...
                <div className="h-8 w-8 bg-gradient-to-br from-blue-600 to-blue-700 rounded-xl flex items-center justify-center">
                    <Building2 className="w-5 h-5 text-while" />
                </div>
              )}
        </div>
        ...
        <div className="absolute bottom-4 left-4 right-4">
            <button className="w-full flex items-center px-3 py-2.5 text-sm font-medium rounded-lg text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-all duration-200" 
                onClick={logout}
                >
