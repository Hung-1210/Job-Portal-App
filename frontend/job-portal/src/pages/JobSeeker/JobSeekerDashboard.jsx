...
useEffect(() => {
  const timeoutId = setTimeout(() => {
    ...
    if (hasFilters) {
      ...
      fetchJobs(); // Gọi API mà không có bộ lọc
    }
  }, 500); // Debounce 500ms để tránh gọi API liên tục

  return () => clearTimeout(timeoutId); // Cleanup timeout khi component unmount
  }, [filters, user]); // Theo dõi sự thay đổi của bộ lọc
            ...
            {jobs.length === 0 ? (
              ...
              <>
                <div
                  className={
                    viewMode === "grid"
                      ? "grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-2 lg:gap-6"
